   ...
   private String callGeminiForElaboration(String title, String description, String snippet) {
    try {
        String prompt = String.format(
                "Summarize this news into 7 to 8 simple paragraphs. " +
    ...
    public ResponseEntity<?> getActiveBlogs() {
        try {
            List<Blog> activeBlogs = blogRepository.findActiveBlogsOrdered();
    ...
    public ResponseEntity<?> getAllBlogs() {
        try {
            List<Blog> allBlogs = blogRepository.findAllBlogsOrdered();
	...
	public ResponseEntity<?> getBlogById(UUID id) {
        try {
            Optional<Blog> optionalBlog = blogRepository.findById(id);
            if (optionalBlog.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred while retrieving the blog with id: " + id);
        }
	}
}
